...
void wrapper(double *pin, int pinlen, double cf, int nrep, double dt, double reptime, int cohc, int cihc, int species, double *ihcout)
{
  ...
  // pxbins = (int) mxGetN(prhs[0]);
  pxbins = pinlen; // pxbins is the length of the input array (pin) [david comment]
  // if (pxbins==1)
  //   mexErrMsgTxt("px must be a row vector\n");

  if (species<1 || species>3)
    printf("Species must be 1 for cat, or 2 or 3 for human.\n");
  ...
  if ((cf<124.9)|(cf>40.1e3))
  {
    printf("cf (= %1.1f Hz) must be between 125 Hz and 40 kHz for cat model\n",cf);
    }
    }
    if (species>1)
    {
    if ((cf<124.9)|(cf>20.1e3))
  {
    printf("cf (= %1.1f Hz) must be between 125 Hz and 20 kHz for human model\n",cf);
    }
    }
  ...
  if ((cohc<0)|(cohc>1))
  {
    printf("cohc (= %d) must be between 0 and 1\n",cohc);
  }

  if ((cihc<0)|(cihc>1))
  {
    printf("cihc (= %d) must be between 0 and 1\n",cihc);
  }
  ...
  px = (double*)malloc(totalstim*sizeof(double));
...
int testfun(float arg, float *array) {
  printf("yo: %.1f\n", arg);
  printf("array 1: %.1f\tarray 2: %.1f\n", array[0], array[1]);
...
{	
	...
	ihcouttmp  = (double*)malloc(totalstim*nrep*sizeof(double));
	    
	mey1 = (double*)malloc(totalstim*sizeof(double));
	mey2 = (double*)malloc(totalstim*sizeof(double));
	mey3 = (double*)malloc(totalstim*sizeof(double));

	tmpgain = (double*)malloc(totalstim*sizeof(double));
